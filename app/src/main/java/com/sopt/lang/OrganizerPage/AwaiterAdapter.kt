 ...
 */
class AwaiterAdapter(var dataList: ArrayList<MeetingParticipantsData>?, var ctx: Context?) : RecyclerView.Adapter<OrganizerPageViewHolder>() {
    ...
    override fun onBindViewHolder(holder: OrganizerPageViewHolder, position: Int) {
        ...
        holder!!.itemView.setOnClickListener(View.OnClickListener { v ->
            //내 모임 눌렀을 때 모임 상세보기
            val intent = Intent(ctx, OrganizerPageDetailActivity::class.java)
            intent.putExtra("key",dataList!!.get(position).meeting_user_id)
